class PsDbDatabase
{
    [string] $DatabaseProvider = "Undefined"
    [int] $DefaultCommandTimeout = 100
	...
	hidden [System.Data.Common.DbCommand] $_sqlCmd
	...
	NewSqlCommand()
	{
		$this.CloseSqlCommand()
		$this._sqlCmd = $this._newSqlCommand()
	}

	######################################################################################################
	CloseSqlCommand()
	{
		if ($this._sqlCmd -ne $null)
		{
			$this._sqlCmd = $null
		}
	}
    ...
    {
		$this.AssureSqlCommand()
		$this._sqlCmd.CommandText = $this._GetPsDbDeployVersionSql
        $version = $this._sqlCmd.ExecuteScalar()
        return $version
    }
        ...
        foreach ($ParsedQuery in $ParsedQueries)
        {
            if ($ParsedQuery.Trim() -ne "")
            {
                ...
                if (! $this.PublishWhatIf)
                {
                    try
                    {
                        $this._sqlCmd.CommandText=$ParsedQuery
                        [void] $this._sqlCmd.ExecuteNonQuery()
                    } 
                    catch
                    {
...
<#  =================================================================================================
	SqlServerDatabase inherits from PsDbDatabase
    ================================================================================================= #>
...
"@
	...
	hidden [string] $_beginTransaction  = @"
SET XACT_ABORT ON
...
"@
    ...
    #-----------------------------------------------------------------------------------------------
    hidden [string] $_endTransactionScript = @"
IF @@ERROR <> 0 AND @@TRANCOUNT >  0 WHILE @@TRANCOUNT>0 ROLLBACK TRANSACTION;
WHILE @@TRANCOUNT > 0 COMMIT TRANSACTION;
"@

    hidden [string] $_rollbackTransactionScript = @"
